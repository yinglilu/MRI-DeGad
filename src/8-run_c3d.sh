...
output_dir='/project/6050199/akhanf/cfmm-bids/data/Lau/degad/derivatives/c3d_new_2'
...
for sub in ${sub_list}; do
    ...
    singularity run /project/6050199/akhanf/singularity/pyushkevich_itksnap_latest.sif c3d ${input_dir}/${sub}/${sub}_acq-gad_rescaled_T1w.nii.gz ${input_dir}/${sub}/${sub}_acq-nongad_normalized_fcm.nii.gz ${input_dir}/${sub}/${sub}_acq-nongad_normalized_fcm.nii.gz -xpa 5 10 -xp ${output_dir}/${sub}/${sub}_samples_31.dat 15x15x15 4500| tail -n 1| awk -F':' '{print $NF}' >>  ${output_dir}/num_patches_31.txt 
...
for sub in ${sub_list}; do
    ...
    singularity run /project/6050199/akhanf/singularity/pyushkevich_itksnap_latest.sif c3d ${input_dir}/${sub}/${sub}_acq-gad_rescaled_T1w.nii.gz ${input_dir}/${sub}/${sub}_acq-nongad_normalized_fcm.nii.gz ${input_dir}/${sub}/${sub}_acq-nongad_normalized_fcm.nii.gz -xpa 5 10 -xp ${output_dir}/${sub}/${sub}_samples_15.dat 7x7x7 450| tail -n 1| awk -F':' '{print $NF}' >>  ${output_dir}/num_patches_15.txt 
